...
V_avg = avg_dict[V]
T_avg = avg_dict[T]
import_music_average= [D_avg,E_avg,V_avg,T_avg]
...
for i in range(count):
    feature_id_list = [id_array[i],d_array[i],e_array[i],v_array[i],t_array[i]]
    feature_list = [d_array[i],e_array[i],v_array[i],t_array[i]]
    #print(feature_list)
...
recommend_music_average = [recommend_average_D,recommend_average_E,recommend_average_V,recommend_average_T]
print('recommend_music_average: ',recommend_music_average)

#グラフ生成
x_1 = [0, 1, 2, 3]            # 系列 1 をプロットするx座標
x_2 = [0.2, 1.2, 2.2, 3.2]  # 系列 2 をプロットするx座標
